steal('steal/build', 'steal/parse').then(function( steal ) {
	...
	scripts.compressors = {
		chomp: function() {
			function Chomper(logger) {
				this.logger = logger || function(){};
				this.src = [];
				this.matches = {};
				/* /\/\/ ?steal:chomp:?([a-zA-Z0-9\)\(]+)?(?:\[([a-zA-Z ]+)\])?/ */
				this.matcher = new RegExp("\\/\\/ ?steal:chomp:?([a-zA-Z0-9\\)\\(]+)?(?:\\[([a-zA-Z ]+)\\])?");
			};

			Chomper.prototype.consolidate = function() {
				for(var tag in this.matches) {
					var item = this.matches[tag];
					if(item.end) {
						//logging
						this.logger('* Removing \''+tag + '\' Line '+(item.start+1)+' to Line '+(item.end+1));
						if(item.comments.length > 0){
							this.logger('* - '+item.comments.join(', '));
						}
						// blank those items in the array
						// we don't splice here, because it will mess with the other indexes
						this.blankify(item.start, item.end);
					}
				}
				return this.src.join("\n");
			};

			Chomper.prototype.blankify = function(start, finish) {
				for(;start <= finish; start++) {
					this.src[start] = '';
				}
			};

			Chomper.prototype.chomp = function(src) {
				this.src = src.split(/\n/);
				this.analyze();
				return this.consolidate();
			};

			Chomper.prototype.record = function(regexpMatch, index) {
				var realLine = index+1;
				var match = regexpMatch[0] || false;
				var tag = regexpMatch[1] || false;
				var comment = regexpMatch[2];
				if(!tag) {
					return this.logger('Found steal:chomp in line '+realLine+' without a tag');
				}
			
				var record = (this.matches[tag] || {
					start: index,
					comments: []
				});

				var existing = (this.matches[tag] || false);
				if(existing) {
					//Acknowledgement:
					//this could trigger on an end value of 0...
					if(existing.end) {
						return this.logger('Duplicate steal:chomp tag end detected in line '+realLine+' (ignoring)');
					}
					record.end = index;
				}
				...
				this.matches[tag] = record;
				var commentLog = comment ? ' ('+comment+')' : '';
			};

			Chomper.prototype.analyze = function() {
				this.logger('Checking '+this.src.length+' lines');
				for(var i = 0; i < this.src.length; i++) {
					var result = this.matcher.exec(this.src[i]);
					if(result) {
						this.record(result, i);
					}
				}
			};

			return function(src) {
				var formatter = new Chomper(steal.print);
				return formatter.chomp(src);
			}
		},
		skip: function() {
				return function(src) {
